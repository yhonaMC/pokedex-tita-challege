...
const validatePokemonData = (pokemon: Pokemon): { isValid: boolean; errors: Record<string, string> } => {
...
export const setPokemonListWithValidation = (pokemonList: Pokemon[]) => (dispatch: Dispatch) => {
  ...
  pokemonList.forEach((pokemon, index) => {
    ...
    if (validation.isValid) {
      ...
      validationErrors[`pokemon_${index}`] = Object.values(validation.errors).join(', ');
    }
  });
