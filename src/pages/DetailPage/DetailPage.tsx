...
import { setCurrentPokemon, setLoading, setError } from '../../store/pokemonSlice';
import { GET_POKEMON_DETAIL } from '../../services/pokemonQueries';
import { getPokemonImageUrl } from '../../utils/pokemonUtils';
import type { Pokemon, GraphQLDetailPokemon, GraphQLPokemonType, GraphQLPokemonStat } from '../../types/pokemon';
...
export const DetailPage = ({ pokemonId, onBack }: DetailPageProps) => {
  ...
  const { loading, error, data } = useQuery<PokemonDetailResponse>(GET_POKEMON_DETAIL, {
  ...
  useEffect(() => {
    if (data?.pokemon_v2_pokemon_by_pk) {
      ...
      const spritesData = pokemonData.pokemon_v2_pokemonsprites?.[0]?.sprites;
      ...
      const transformedPokemon: Pokemon = {
        ...
        types: pokemonData.pokemon_v2_pokemontypes?.map((typeData: GraphQLPokemonType) => ({
          type: {
            name: typeData.pokemon_v2_type.name,
          },
        })) ?? [],
        stats: pokemonData.pokemon_v2_pokemonstats?.map((statData: GraphQLPokemonStat) => ({
      ...
      const flavorText = pokemonData.pokemon_v2_pokemonspecy?.pokemon_v2_pokemonspeciesflavortexts?.[0]?.flavor_text;
      if (flavorText) {
        setDescription(flavorText.replace(/\f/g, ' ').replace(/\n/g, ' '));
      }

      const genusData = pokemonData.pokemon_v2_pokemonspecy?.pokemon_v2_pokemonspeciesnames?.[0]?.genus;
      if (genusData) {
        setGenus(genusData);
      }
    }
  ...
  const handleToggleFavorite = () => {
    if (currentPokemon) {
      const pokemonImage = currentPokemon.sprites.other['official-artwork'].front_default || currentPokemon.sprites.front_default;
  ...
  const isFavorite = currentPokemon ? favorites.some(fav => fav.id === currentPokemon.id) : false;
    ...
    return (
      <div className="detail-page">
        ...
        <div className="detail-page__loading">
          ...
          />
          <div className="detail-page__loading-text">Loading Pokémon details...</div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="detail-page">
        ...
        <div className="detail-page__not-found">
          <div className="detail-page__not-found-text">
            Pokémon not found
          </div>
        </div>
      </div>
    ...
    <div className="detail-page">
      <div className="detail-page__header">
        <Header 
          showBackButton 
          onBack={onBack}
          title={`${currentPokemon.name} #${currentPokemon.id.toString().padStart(3, '0')}`}
        />
      </div>
